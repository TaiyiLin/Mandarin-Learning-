...
class SRChatRoomAdapter : ListAdapter<DataItem, RecyclerView.ViewHolder>(DiffCallback) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when (viewType) {
            ...
            MSG_FROM_RECEIVER -> {
                SRChatRoomAdapter.ReceiverRightViewHolder.from(parent)
            }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (holder) {
            is SRChatRoomAdapter.SenderLeftViewHolder -> {
                val msgItem = getItem(position) as DataItem.SenderMsg
                holder.bind(msgItem.message)
            }
            is SRChatRoomAdapter.ReceiverRightViewHolder -> {
...
sealed class DataItem {
    ...
    data class SenderMsg(val message: Message) :
        com.taiyilin.mandarinlearning.sentenceReordering.DataItem() {
